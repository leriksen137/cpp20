...
inline void show(char const* title, CompilerFeature(&features)[N]) {
    if constexpr (print.titles) {
        std::cout << '\n' << std::left << title << '\n';
    }
    if constexpr (print.sorted_by_value) {
...
inline void print_all_comiler_features() {
    if constexpr (print.general_features) show("C++ GENERAL", cxx);
    if constexpr (print.cxx11 && print.core_features) show("C++11 CORE", cxx11);
    if constexpr (print.cxx14 && print.core_features) show("C++14 CORE", cxx14);
    if constexpr (print.cxx14 && print.lib_features) show("C++14 LIB", cxx14lib);
    if constexpr (print.cxx17 && print.core_features) show("C++17 CORE", cxx17);
    if constexpr (print.cxx17 && print.lib_features) show("C++17 LIB", cxx17lib);
    if constexpr (print.cxx20 && print.core_features) show("C++20 CORE", cxx20);
    if constexpr (print.cxx20 && print.lib_features) show("C++20 LIB", cxx20lib);
    if constexpr (print.cxx23 && print.core_features) show("C++23 CORE", cxx23);
    if constexpr (print.cxx23 && print.lib_features) show("C++23 LIB", cxx23lib);
    if constexpr (print.attributes) show("ATTRIBUTES", attributes);
}
